...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    console.log('Checking Mailchimp for email...');
    ...
    if (checkResponse.ok) {
      ...
      console.log('Member found, status:', memberData.status);
    ...
    console.error('Error in subscription handler:', error.message || error);
    return res.status(500).json({ 
      title: 'Server Error',
